...
const currency = z.enum(["DOP", "USD", "EUR", "GBP", "CAD"]);
const exchangeRate = z.number().finite().positive().max(1_000_000);
...
const createTransaction = z.object({
  ...
  exchange_rate_to_base: exchangeRate.optional(),
  spent_at: dateText,
...
const updateAccount = z.object({
  ...
  balance: signedMoney.optional(),
  currency: currency.optional(),
  exchange_rate_to_base: exchangeRate.nullable().optional(),
...
export function actionLabel(action: AssistantAction & { preview?: string }): string {
  ...
  switch (action.type) {
    case "create_transaction": return `Crear ${action.kind}: ${action.concept} (${action.amount} ${action.currency || "DOP"})`;
    ...
    case "upsert_budget": return `Guardar presupuesto de ${action.limit_amount} DOP`;
