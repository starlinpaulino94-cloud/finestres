...
async function resolveCurrencyForAccounts(
  userId: string,
  originId: string | undefined,
  destinationId: string | undefined,
  requestedCurrency?: string,
  requestedRate?: number | null,
  fallback?: { currency?: string | null; exchange_rate_to_base?: number | null }
) {
  const origin = originId ? await owned("bank_account", originId, userId) : null;
  const destination = destinationId ? await owned("bank_account", destinationId, userId) : null;
  const originCurrency = normalizeCurrency(origin?.currency || BASE_CURRENCY);
  const currency = normalizeCurrency(requestedCurrency || origin?.currency || fallback?.currency || BASE_CURRENCY);
  if (origin && currency !== originCurrency) {
    throw new AssistantActionError("La moneda del movimiento debe coincidir con la moneda de la cuenta");
  }
  if (destination && currency !== normalizeCurrency(destination.currency)) {
    throw new AssistantActionError("Las transferencias entre monedas distintas requieren dos movimientos separados");
  }
  const exchangeRate = exchangeRateToBase(
    currency,
    requestedRate ?? origin?.exchange_rate_to_base ?? fallback?.exchange_rate_to_base
  );
  if (currency !== BASE_CURRENCY && !exchangeRate) {
    throw new AssistantActionError("La acción en moneda extranjera necesita tasa hacia DOP");
  }
  ...
  for (const action of actions) {
    switch (action.type) {
      case "create_transaction": {
        ...
        await resolveCurrencyForAccounts(
          userId,
          action.bank_account_id,
          meta.needsDestination ? action.transfer_account_id : undefined,
          action.currency,
          action.exchange_rate_to_base
        );
        break;
      }
      case "update_transaction": {
        ...
        await resolveCurrencyForAccounts(
          userId,
          origin,
          meta.needsDestination ? destination : undefined,
          action.currency,
          action.exchange_rate_to_base,
          current
        );
...
export async function executeAssistantAction(
  ...
  switch (action.type) {
    case "create_transaction": {
      ...
      const money = await resolveCurrencyForAccounts(
        userId,
        action.bank_account_id,
        meta.needsDestination ? action.transfer_account_id : undefined,
        action.currency,
      ...
      const data = await createOnce("transaction", userId, action.actionId, {
        ...
        amount_base: amountToBase(action.amount, money.currency, money.exchangeRate),
        currency: money.currency,
        exchange_rate_to_base: money.exchangeRate,
        kind: action.kind,
    ...
    case "update_transaction": {
      ...
      const money = await resolveCurrencyForAccounts(
        userId,
        sourceAccount,
        meta.needsDestination ? targetAccount : undefined,
        action.currency,
        action.exchange_rate_to_base,
        current
      );
      ...
      update.exchange_rate_to_base = money.exchangeRate;
      update.amount_base = amountToBase(action.amount ?? current.amount, money.currency, money.exchangeRate);
    ...
    case "create_account": {
      const currency = normalizeCurrency(action.currency || BASE_CURRENCY);
      const exchangeRate = exchangeRateToBase(currency, action.exchange_rate_to_base);
      if (currency !== BASE_CURRENCY && !exchangeRate) {
        throw new AssistantActionError("Indica la tasa hacia DOP para esta cuenta");
      }
    ...
    case "update_account": {
      const current = await owned("bank_account", action.id, userId);
      const { type: _type, id: _id, actionId: _actionId, preview: _preview, ...fields } = action;
      const currency = normalizeCurrency(action.currency || current.currency || BASE_CURRENCY);
      const exchangeRate = exchangeRateToBase(
        currency,
        action.exchange_rate_to_base ?? current.exchange_rate_to_base
      );
      if (currency !== BASE_CURRENCY && !exchangeRate) {
        throw new AssistantActionError("Indica la tasa hacia DOP para esta cuenta");
      }
