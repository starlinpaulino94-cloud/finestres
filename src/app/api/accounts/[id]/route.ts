...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.currency !== undefined || body.exchange_rate_to_base !== undefined) {
      const currency = normalizeCurrency(body.currency || owner.record.currency);
      const exchangeRate = exchangeRateToBase(
        currency,
        body.exchange_rate_to_base ?? owner.record.exchange_rate_to_base
      );
