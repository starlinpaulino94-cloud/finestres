...
export async function POST(req: Request) {
  try {
    ...
    if (parsed.data.format === "excel") {
      ...
      const summary = [
        "",
        ["Total gastos", "", "", "", "", "", totalSpent.toFixed(2).replace(".", ",")].map(csvCell).join(";"),
        ["Total ingresos", "", "", "", "", "", totalIncome.toFixed(2).replace(".", ",")].map(csvCell).join(";"),
        ["Balance", "", "", "", "", "", (totalIncome - totalSpent).toFixed(2).replace(".", ",")].map(csvCell).join(";"),
      ];
